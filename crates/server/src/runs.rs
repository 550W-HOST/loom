...
mod tests {
    ...
    /// arrives after the deadline reaped the run cannot be bound. An id with
    /// nothing to bind it to is a claim loom cannot use: it can never be
    /// resumed, so recording it only leaves a value that reads like a
    /// conversation. Nothing is learned, and the next turn opens a session of
    /// its own.
    #[tokio::test]
