...
/**
 * Follows `$ref`s inside one document, so an extension can reach a shared
 * definition the way a validator would.
 */
function resolveRef(
  node: JsonValue | undefined,
  defs: Record<string, JsonValue> | undefined,
): JsonValue | undefined {
  let current = node;
  const seen = new Set<string>();
  while (current && typeof current === "object" && !Array.isArray(current)) {
    const ref = (current as Record<string, JsonValue>).$ref;
    if (typeof ref !== "string" || seen.has(ref)) {
      return current;
    }
    seen.add(ref);
    current = defs?.[ref.split("/").pop() ?? ""];
  }
  return current;
}

/**
 * The timeline response's loom-side additions.
 *
 * bb's contract is the ported client's vocabulary, and loom adds two facts to
 * one response that bb's daemon kept implicit: which generation the sequences
 * belong to, and how much of the conversation the server can offer. A cursor is
 * only meaningful inside a generation, and "no rows" has to be distinguishable
 * from "could not load".
 *
 * It also has to relax one thing. A restored conversation carries no
 * timestamps — an agent's replay has none, and the moment the server loaded it
 * is not the moment anything in it happened — so the three time fields accept
 * null. Every row shape keeps its identity in every other respect, which is the
 * point: a row must not look different because of where it came from.
 *
 * The additions are applied here rather than to the committed artifact because
 * `contracts/bb` is regenerated byte for byte in CI: anything loom adds has to
 * come out of the exporter.
 */
function applyLoomExtensions(serverApi: JsonValue): JsonValue {
  const document = serverApi as {
  ...
  const route = document.routes?.find((r) => r.id === "threads.timeline");
  const schema = route?.responses?.[0]?.schema;
  if (!schema) {
  ...
  if (!properties || !Array.isArray(required)) {
    throw new Error(
      "loom extensions: threads.timeline response is not an object schema",
    );
  }
  ...
  properties.history = {
    ...
    properties: {
      status: {
        enum: ["loading", "partial", "ready", "stale", "unavailable"],
        description:
          "How much of the conversation the server can offer, and whether a newer view is being loaded.",
      },
      complete: {
        type: "boolean",
        description:
          "Whether the rows are the whole conversation. A ready entry with no rows is a real empty conversation; anything else is partial.",
      },
      reason: {
        type: ["string", "null"],
        description:
  ...
  const rows = resolveRef(properties.rows, defs) as
    | { items?: JsonValue }
    | undefined;
  const items = resolveRef(rows?.items, defs) as
    | { anyOf?: JsonValue[] }
    | undefined;
  const branches = items?.anyOf;
  if (!Array.isArray(branches) || branches.length === 0) {
    throw new Error("loom extensions: threads.timeline rows have no branches");
  }
  for (const branch of branches) {
    const resolved = resolveRef(branch as JsonValue, defs) as
      | { properties?: Record<string, JsonValue> }
      | undefined;
    const rowProperties = resolved?.properties;
    if (!rowProperties) {
      continue;
    }
    for (const name of ["startedAt", "createdAt", "completedAt"]) {
      const field = rowProperties[name] as { type?: JsonValue } | undefined;
...
async function main(): Promise<void> {
  ...
  const serverApi = applyLoomExtensions(buildServerApi(routes, named.schemas));
