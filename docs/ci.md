...
| `ui` | `UI typecheck, tests, bundle and provenance` | `pnpm install --frozen-lockfile`, `pnpm run typecheck`, `pnpm run test`, a fresh build reproduces both the reference `ui/app.js` and the product app build, `pnpm run ui:boundary` audits the product runtime, and `node scripts/check-ui-provenance.mjs` verifies source/package/contract hashes and the import closure |
