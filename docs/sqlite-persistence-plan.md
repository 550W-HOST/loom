...
`cacheInstance`）；A1 的**拒绝**半部分已落地（`830150f`：发送/重试/队列/自动化四条路径都
拒绝并保留原映射）。A1 的"用户指定 host 的重新绑定"与"新建会话"两个动作、A3 的 worker 侧
串行互斥尚未实现。

- **A1 旧 binding 不得静默换 session**：已有 `provider_session_id` 但缺可信 host 的线程，
  发送与重试返回明确的 `session_rebind_required`，保留原映射，不新开 session；随后提供
  用户指定 host 的重新绑定动作（经一次 ACP 加载验证成功后才保存），"新建会话"作为独立动作。
