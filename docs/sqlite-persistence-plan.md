...
`cacheInstance`）；A1 的**拒绝**已落地（`830150f`：发送/重试/队列/自动化四条路径都拒绝并
保留原映射），其两个恢复动作按上条决定不做；干净部署下该状态已不可达（`53c1cb3` 修掉
identity 晚于 run 回收的活路径）。**A3 的 worker 侧串行互斥尚未实现**，是阶段 A 剩下的
最后一项。

- **A1 旧 binding 不得静默换 session**：已有 `provider_session_id` 但缺可信 host 的线程，
  发送与重试返回明确的 `session_rebind_required`，保留原映射，不新开 session。
  **恢复动作不做**（2026-09-20 决定）：产品只面向干净安装的新用户，不承担旧数据适配，
  因此"用户指定 host 的重新绑定"和"新建会话"两个入口都不实现；真出现这种 thread 时，
  删除 thread 是唯一出路。干净部署下该状态已不可达——唯一活路径（worker 的 identity
  上报晚于 run 被回收）已修掉（`53c1cb3`）。
