...
**⚠️ 这一步会改变发布工具链（已完成）。** `rusqlite` 的 `bundled` 会为目标平台编译 SQLite 的 C 源码，因此构建脚本需要目标平台的 C 编译器（与选哪个引擎无关；turso 时期同样缺，只是它的 `simsimd` 把缺失吞成 warning、到链接才报 `cannot find -lsimsimd`）。做法改为**按架构选 runner**：`release.yml` 与 CI 的 `release-targets` 任务各自在 `ubuntu-24.04`（x86_64）和 `ubuntu-24.04-arm`（aarch64）上构建，用 Ubuntu 自带的 `musl-tools`（`CC_<target>=musl-gcc`），不下载任何第三方工具链——`.github/actions/install-musl-c-compiler` 是两条流水线共用的那一步。链接方式一行未改（aarch64 仍是 `rust-lld`）。中途试过下载 musl.cc 的 musl-cross-make 档案，实测在 GitHub Actions 上必然失败（`curl: (28) ... musl.cc port 443`，musl.cc 明确整段封禁 GitHub Actions），已放弃。副产品：aarch64 产物现在能在同架构 runner 上**真正跑起来**，因此两个目标都过完整的 `scripts/verify-release-binaries.sh`（两种角色启动、内嵌 app、worker 入网、契约写读），不再只是 ELF 检查。细节见 `docs/releasing.md` 的 “The C compiler the build scripts need”。
