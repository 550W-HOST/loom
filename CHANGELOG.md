...
- The Codex tab points at the bridge it runs: the provider is `codex-acp` (npm `@agentclientprotocol/codex-acp`), so its install and sign-in hints no longer describe a Codex CLI that loom never launches.
