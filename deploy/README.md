...
- The binary, from either a built checkout (`cargo build --release -p loom`) or a
  ...
  `loom-daemon` to it: one file, one role per invocation.
...
| Asset | |
| --- | --- |
| `loom-<target>` | the binary: `loom server` or `loom daemon` |
...
`loom-server` and `loom-daemon` to it, writes the units and the environment
...
newer protocol the daemon fetches the artifact that server hosts — the
`loom-daemon` name `install.sh` leaves beside `loom` — verifies it and restarts
itself ([`../docs/upgrades.md`](../docs/upgrades.md) § Daemon self-update).
`install.sh` puts `loom` in `/usr/local/bin` with `loom-daemon` as a symlink to
...
sudo ln -sfn loom /usr/local/bin/loom-daemon
